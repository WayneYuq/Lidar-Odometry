...
#include "lidar_localization/models/registration/voxel_grid_covariance_omp.h"
...
MYNDTRegistration::MYNDTRegistration(const YAML::Node& node)
    :myndt_ptr_(new pclomp::NormalDistributionsTransform<CloudData::POINT, CloudData::POINT>()) {
    
    float res = node["res"].as<float>();
    float step_size = node["step_size"].as<float>();
    float trans_eps = node["trans_eps"].as<float>();
    int max_iter = node["max_iter"].as<int>();
    pclomp::NeighborSearchMethod sear_methods = pclomp::NeighborSearchMethod(node["sear_methods"].as<int>());

    SetRegistrationParam(res, step_size, trans_eps, max_iter, sear_methods);
}

MYNDTRegistration::MYNDTRegistration(float res, float step_size, float trans_eps, int max_iter, pclomp::NeighborSearchMethod sear_methods)
    :myndt_ptr_(new pclomp::NormalDistributionsTransform<CloudData::POINT, CloudData::POINT>()) {
...
bool MYNDTRegistration::ScanMatch(const CloudData::CLOUD_PTR& input_source, 
    ...
    myndt_ptr_->setNumThreads(omp_get_max_threads());
